...
public class IGEOPlaces implements Serializable {
	...
	/**
	 * Gera o código SQL para uma query à base de dados local para obter os locais.
	 * @return Código da query SQL.
	 */
	public String generateSQL(){
		String result = "";
		
		//System.out.println("start create db script");
		
		try {
			DataInputStream in = new DataInputStream(is);
			BufferedReader br = new BufferedReader(new InputStreamReader(in,"ISO-8859-1"));

			String lastPrefixDist = null;
			String lastPrefixConc = null;
			...
			while((line = br.readLine()) !=null){
				...
				if(lastPrefixDist==null){  //se ainda n existe prexifo do distrito
					lastPrefixDist = cod;
					parent = null;
				}
					...
					if(lastPrefixConc!=null){  //se já existe prefixo do concelho
						if(cod.startsWith(lastPrefixConc)){ //se estamos no mesmo concelho
							parent = lastPrefixConc;
						}
						else if(cod.startsWith(lastPrefixDist)){  //se mudamos de concelho mas estamos no mesmo distrito
							parent = lastPrefixDist;
							lastPrefixConc = cod;
						}
						else {  //se não estamos nem no mesmo concelho nem no mesmo distrito
							parent = null;
							lastPrefixDist = cod;
						}
					}
					else {
						if(cod.startsWith(lastPrefixDist)){  //estamos no mesmo distrito no primeiro concelho
							parent = lastPrefixDist;
							lastPrefixConc = cod;
						}
					}
				...
				result += "INSERT INTO Localizations VALUES(\""+cod+"\", \""+name+"\", \""+parent+"\", 1);\n";
				
				//System.out.println("INSERT INTO Localizations VALUES("+cod+", \""+name+"\", "+parent+", 1);\n");
				
			}

			br.close();
			
			//System.out.println("end create db script");
